...
- **개설·문항 편집:** role=MENTOR + 소유 멘토(자기 모임)만. 타인 모임 편집 403.
- **운영 허브 조회:** 자기 모임만(소유 경계 403). 신청자·설문 응답 열람은 소유 멘토/관리자만(U4/U8이 각 403 집행).
- **모집 목록:** RECRUITING 모임은 로그인 사용자에 공개(민감정보 아님).

## 데이터 보호

- 사전설문 문항 틀은 모임 참여자·멘토·관리자 범위. 응답은 U8 소유(권한 U8).
