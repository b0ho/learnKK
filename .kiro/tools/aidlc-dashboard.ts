...
// Acronym scopes that Title-casing would otherwise mangle.
...
// Values that arrive already display-formatted (Greenfield, Comprehensive,
// Standard, …) contain an uppercase letter and pass through untouched. Lower-
// case slugs (enterprise, security-patch, custom scopes) get Title-cased, with
// a small override table for acronyms. This handles any scope, including
// user-defined ones, without a per-scope entry.
function localizedConfig(value: string): string {
  if (!value || /[A-Z]/.test(value)) return value;
  return CONFIG_LABELS[value] ?? titleCase(value);
}
  ...
  const stages: DashboardStage[] = graph
    ...
    .map((entry) => {
      ...
      return {
        ...
        condition: localizedCondition(entry.execution, status),
  ...
  <script>
    ...
    function updateDashboard(content, graph, context) {
      ...
      document.querySelectorAll(".stage-card").forEach((card) => {
        ...
        if (graphStage) {
          ...
          const values = {
            condition: stateStage.status === "skipped"
              ? "현재 Scope의 실행 계획에서 제외된 Stage입니다."
              : graphStage.execution === "ALWAYS"
                ? "현재 Scope의 실행 계획에 포함되는 필수 Stage입니다."
                : "Project Type과 Scope의 실행 조건이 충족될 때 수행합니다.",
      ...
      const currentName = (current && current.name) || currentSlug;
      const nextName = (next && next.name) || "Workflow 완료";
    ...
    document.querySelectorAll("[data-guide]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const name = graphStage?.name || (card ? card.querySelector("h3").textContent : slug);
